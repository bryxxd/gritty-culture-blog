...
const { data: article } = await useAsyncData(`article-${route.params.slug}`, () => getArticleBySlug(route.params.slug as string));
const { data: moreArticles } = await useAsyncData(`articles-${route.params.slug}`, () => getArticles(3, article.value?.databaseId ? [article.value.databaseId] : []));
</script>
<template>
    <div class="layout">
        <HeaderSection custom-class="text-[clamp(40px,10vw,110px)]/[95%] p-[89px_0_24px] md:pt-[118px]"
            :title="article?.title" />
        <p
        ...
        <figure>
            <NuxtImg :src="article?.articleFields?.article_header?.node?.sourceUrl" :alt="article?.articleFields?.article_header?.node?.altText" placeholder
                class="w-full rounded-lg mt-[110px] object-cover md:mt-[170px] h-[320px] md:h-[600px] lg:h-[700px]" />
        </figure>
    </div>

    <section class="layout">
        <div class="py-[74px] md:py-[130px] px-10 text-xl font-semibold flex flex-col md:flex-row gap-10">
            <p class="flex-shrink-0 w-auto md:w-[200px]">Why this exists</p>
            <p class="tracking-tight">{{ article?.articleFields?.article_intro }}</p>
        </div>
    </section>

    <section class="layout text-paragraph-2">
        <div class="bg-background-3 rounded-lg">
            <div class="md:max-w-[600px] mx-auto py-[64px] md:py-[120px] px-5">
                ...
                <div class="mt-[64px] md:mt-[120px] pt-10 border-t-[1px] border-dashed border-divider-1">
                    <img src="~/assets/img/article/footer-icons.png" class="max-w-[140px] mx-auto" alt="" />
                </div>
            </div>
        </div>
    </section>

    <section class="layout px-10px text-headline-1 mt-[10px]">
        <div
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <template v-for="article in moreArticles" :key="article.id">
                    <NuxtLink :to="{ name: 'article-slug', params: { slug: article.slug } }"  class="font-semibold group">
